...
- **記事が0件の日も正常系**。アイテム0件の有効なRSSを生成する。
  Power Automateの07:00フローも0件を取得するため、条件分岐により投稿は行われない
...
**毎朝07:00にフィードを取得し、記事があればTeamsへ投稿する**フローを作る。
RSSトリガーではなくスケジュール実行+フィード取得方式なので、投稿時刻が正確で、
0件の日は単に何も送信されない(フローは走るが投稿アクションがスキップされる)。
...
1. **トリガー: 「繰り返し」(Recurrence)**
   ...
   - 時刻: `07:00`
2. **アクション: 「RSS」→「すべてのRSSフィードアイテムを一覧表示」**
   - RSSフィードのURL: `https://yuzu-krs.github.io/copilot-teams-feed/rss/copilot.xml`
   - `since` は空のまま(フィード自体が「最新1日分だけ」なので全件取得=その日の分)
3. **条件(Condition)**: 記事があるかどうかの判定
   - 左辺(式): `length(body('すべてのRSSフィードアイテムを一覧表示')?['value'])`
   - 演算子: `次の値より大きい` / 右辺: `0`
...
「チャットまたはチャネルでカードを投稿」の各フィールド:

| フィールド | 設定値 |
|---|---|
| 見出し(Headline) | `<h3>📰 <a href="[リンク]">[アイテム タイトル]</a></h3><p>[概要]</p>`(動的なコンテンツ `[アイテム タイトル]` `[概要]` `[リンク]` を挿入) |
| 概要(Summary) | `[アイテム タイトル]`(通知プレビュー用) |

Teamsでの表示イメージ:

```
📰 Copilot 予算増加リクエストが一般提供開始   ← クリックで元記事へ
これまで、Copilot AI クレジットを使い切ったメンバーは、クレジットを
消費する機能が利用できなくなっていました。今回のリリースでは…(2〜3文)
```

- 記事が複数ある日は**古い記事から順に1記事1投稿**される
- 投稿本文はパイプラインが生成した日本語要約(2〜3文)。製品名・機能名は英語のまま

### 注意

- フローは**毎朝1回だけ**実行する前提(フィードは毎朝6:50に前日分へ完全置換される)。
  同じ日に手動実行すると同じ記事を二重投稿するので注意
- 06:50のActions実行が失敗していても、PAは前の日のフィード内容を取得してしまうため、
  念のためActionsの成功を確認してから詳細を確認するとよい
